...
enum {
  ...
  AM_TOPOLOGYBUILD = 138,
  AM_PRIVATERADIO   = 139,
  ATTACK_DROP_RATE = 20, //percentage of packets to be dropped
  ATTACK_NODE_ID = 14, //ID of an attacker
  ATTACK_NODE_ID2 = 10,
  EXPERIMENT_SOURCE_ID =15,
  EXPERIMENT_SOURCE_ID2 = 31,
  EXPERIMENT_EVENT_PERIOD = 2048, //event frequency
  EXPERIMENT_MOVEMENT_COUNT = 100, //number of generated events
  EXPERIMENT_START = 0,
  EXPERIMENT_BOOTSTRAP = 1,
  EXPERIMENT_RUNNING = 2,
  EXPERIMENT_END  = 3,
  EXPERIMENT_COUNT = 100, // number of experiments to conduct
  EXPERIMENT_BOOTSTRAP_TIME = 81920, //80s 
  EXPERIMENT_BOOTSTRAP_MSG_COUNT = 100, //  should be cca (EXPERIMENT_BOOTSTRAP_TIME - DELAY_TIME) / SENT_TIME
  SIMULATION_TIME = 245760, // 614400, // 10 minutes //7372800,	    //until writing to logFile if regular node
  BS_RUN_TIME     = 7372800,    	    //until writing to logFile if node is a basestation 
  DATA_GENERATOR_FIRST_SHOT = 100000, 
  DATA_GENERATOR_NEXT_SHOTS = 20000,  //data generating period
  NUM_OF_GENERATED_SD_MSG   = 1,	    //maximum number of generated "sensor" messages
  POLICEMAN_TIMER_MESSAGE_MILLI = 50000,
  IDS_THRESHOLD_TIME = 10240,    //10s
  IDS_THRESHOLD_SENT = 10,	//not used so far
  IDS_MIN_SENT = 50,   //minimum messates forwarded so we can start to calculate drop rate 
  IDS_DETECTION_WINDOW = 307200, //300s	
  SEND_TIME = 512,   //sending every 0.5s
  DELAY_TIME = 30720, //synchronization 30s 
  COUNTER_SYNC_TRIALS = 5,
  KEY_LENGTH = 16,
  SEND_BUFFER_LEN = 10,
  ...
  COUNT_SEND = 2,
  DUMMY_SEND = 3,
  EXPECTED_BUFF_LEN = 32,
  MSG_LEN = 3,
  PACKET_LEN = NONCE_LEN+MAC_LEN+MSG_LEN,
  REC_STATE_FROM_ALL=0,
  REC_STATE_FROM_CHILD=1,
...
#define EXPERIMENT_IDS_THRESHOLD 0.95
...
typedef nx_struct ChildData {
	nx_uint16_t nodeId;
	nx_uint16_t counter;
	PL_key_t	encKey;
	PL_key_t 	macKey;
	nx_uint16_t  modifCount;
	nx_uint16_t	 receiveCount;
} ChildData_t;

void clearChildData(ChildData_t* childData)
{
	childData->nodeId=0;
	childData->counter=0;
	memset(childData->encKey.keyValue,0,sizeof(nx_uint8_t)*KEY_LENGTH);
	childData->encKey.keyType = KEY_ENC;
	memset(childData->macKey.keyValue,0,sizeof(nx_uint8_t)*KEY_LENGTH);
	childData->encKey.keyType = KEY_MAC;
	childData->modifCount = 0;	
	childData->receiveCount = 0;	
}

typedef struct ExpectedMAC {
	uint8_t 	isEmpty;
	uint8_t MAC[MAC_LEN];
	uint32_t timeSent;
	uint8_t numSent; 
...
void clearExpectedMAC(ExpectedMAC_t* mac)
{
	mac->isEmpty=TRUE;
	memset(mac->MAC,0,MAC_LEN);
}
...
typedef struct ParentData {
	...
	uint64_t	sentMask;
	float		thresholdIDS;
	ExpectedMAC_t expMAC[EXPECTED_BUFF_LEN];
	ExpectedMAC_t* storeMACptr;
	PL_key_t	encKey;
	PL_key_t 	macKey;
...
void clearParentData(ParentData_t* parentData)
{
	uint8_t i;
	parentData->nodeId=0;
	parentData->counter=0;
	parentData->storeMACidx=0;
	parentData->dropCount = 0;
	parentData->sentCount = 0;
	parentData->sentMask=0;
	parentData->thresholdIDS=0;
	for(i=0;i<EXPECTED_BUFF_LEN;i++)
	{
		clearExpectedMAC(&(parentData->expMAC[i]));
	}
	parentData->storeMACptr=parentData->expMAC;
...
typedef struct CombinedData {
	ChildData_t childList[MAX_CHILD_COUNT];
	ParentData_t parentList[MAX_PARENT_COUNT];
